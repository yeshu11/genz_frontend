...
const SuperAdminDeletedJobs = () => {
  ...
  const fetchDeletedJobs = async () => {
    try {
      const response = await fetch("http://localhost:3001/super_admin/deleted_jobs");
  ...
  const deleteJob = async (jobId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/super_admin/deleted_jobs/${jobId}`, {
        method: "DELETE",
      });
  ...
  const restoreJob = async (jobId) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/super_admin/deleted_jobs/${jobId}/restore`, {
        method: "POST",
      });
  ...
  return (
    <div className="p-8 bg-gradient-to-br from-gray-800 to-gray-900 min-h-screen text-white">
      <h1 className="text-3xl font-bold text-center mb-6">Super Admin - Deleted Jobs</h1>

      <div className="grid grid-cols-3 gap-6">
        {deletedJobs.map((job) => (
          <div
            key={job.id}
            className="relative p-6 border rounded-lg shadow-lg bg-white text-black hover:bg-gradient-to-r from-red-400 to-red-600 transition-all duration-300 transform hover:scale-105 cursor-pointer h-32"
            onClick={(e) => {
      ...
      {selectedJob && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center p-6">
          <div className="bg-white text-black w-[95%] max-w-7xl p-6 rounded-lg shadow-xl transform transition-all scale-105">
            ...
            <h2 className="text-2xl font-semibold text-center text-red-600">{selectedJob.title} Details & Resumes</h2>
...
export default SuperAdminDeletedJobs;
