...
const SuperAdminResumeTechwise = () => {
  ...
  useEffect(() => {
    const fetchResumes = async () => {
      try {
        const response = await fetch("http://localhost:3001/resumes"); // API for resumes
        if (!response.ok) {
        ...
        const groupedData = data.reduce((acc, resume) => {
          const jobTitle = resume.job_title?.trim() || "Unknown Job";
          if (!acc[jobTitle]) {
            acc[jobTitle] = [];
          }
          acc[jobTitle].push(resume);
          return acc;
        }, {});
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold text-center mb-6">Super Admin Resume Techwise</h1>
...
export default SuperAdminResumeTechwise;
