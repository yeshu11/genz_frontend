...
const SuperAdminDashboard = () => {
  ...
  const handleLogout = () => {
    fetch("http://localhost:3001/admin_super/logout", {
      method: "DELETE",
      credentials: "include",
    }).then(() => {
      localStorage.removeItem("admin_super");
  ...
  return (
    <div className={`h-screen w-screen flex justify-center p-8 overflow-hidden transition-all duration-300 ${
      darkMode ? "bg-gray-900" : "bg-gradient-to-r from-blue-500 to-green-700"
    }`}>
      <div className={`w-full max-w-[120%] h-full shadow-2xl rounded-3xl flex overflow-hidden transition-all duration-300 ${
        ...
        <div className={`w-1/6 p-6 rounded-l-3xl shadow-lg transition-all duration-300 ${
          ...
          <div className="flex justify-center">
            <img
              src={darkMode ? "/updated_Genz_cover_White_text-removebg.png" : "/updated_Genz_Cover_black__text-removebg.png"}
              alt="GenZ Logo"
          ...
          <ul className="mt-6 space-y-4 text-sm">
            <li>
              <button onClick={() => setActiveTab("dashboard")} className={`flex items-center gap-2 py-2 px-3 w-full rounded-lg transition ${
            ...
            <li>
              <button onClick={() => setActiveTab("profile")} className={`flex items-center gap-2 py-2 px-3 w-full rounded-lg transition ${
        ...
        <div className="w-5/6 h-full flex-1 relative overflow-hidden">
          <div className="absolute top-5 right-8 flex items-center justify-end gap-4">
            <div className={`shadow-lg rounded-full p-3 w-[130px] h-14 flex items-center justify-end gap-x-3 transition-all ${
              ...
              <button onClick={toggleDarkMode} className="text-2xl cursor-pointer transition-all">
                {darkMode ? "☀️" : "🌙"}
              </button>
              <Menu as="div" className="relative">
                <MenuButton className="flex items-center">
                  <img src="/test_profile.png" alt="Profile" className="w-10 h-10 rounded-full object-cover cursor-pointer border-2 border-gray-200" />
                </MenuButton>
                <Menu.Items className={`absolute right-0 mt-3 w-56 shadow-md rounded-lg border z-50 overflow-hidden py-2 transition-all ${
                  ...
                  <div className="px-4 py-2 text-sm font-semibold">👋 Hey, {adminSuper ? adminSuper.email : "Super Admin"}!</div>
                  <Menu.Item>
                    {({ active }) => (
                      <button className={`px-4 py-2 w-full text-left ${active ? "bg-gray-200" : ""}`}>Profile Settings</button>
                    )}
                  </Menu.Item>
                  <Menu.Item>
                    {({ active }) => (
